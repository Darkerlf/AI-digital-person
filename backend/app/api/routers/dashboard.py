...
def hot_spots(_: object = Depends(require_ops_roles), db: Session = Depends(get_db)):
    return DashboardService(db).get_hot_spots()


@router.get("/behavior-trends")
def behavior_trends(_: object = Depends(require_ops_roles), db: Session = Depends(get_db)):
    return DashboardService(db).get_behavior_trends()


@router.get("/feedback-report", response_model=FeedbackReport)
def feedback_report(_: object = Depends(require_ops_roles), db: Session = Depends(get_db)):
