...
@router.get("")
def list_faqs(_: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
    return {"items": KnowledgeService(db).list_faqs()}


@router.post("", status_code=status.HTTP_201_CREATED)
def create_faq(payload: FAQCreate, _: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
...
def delete_faq(faq_id: int, _: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
