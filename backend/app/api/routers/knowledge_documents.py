...
def list_documents(_: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
    return KnowledgeService(db).list_documents()


@router.get("/{document_id}", response_model=KnowledgeDocumentRead)
def get_document(document_id: int, _: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
    return KnowledgeService(db).get_document(document_id)


@router.get("/{document_id}/chunks")
def list_chunks(document_id: int, _: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
