...
def list_areas(_: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
    return ScenicAreaService(db).list_all()


@router.get("/{area_id}", response_model=ScenicAreaRead)
def get_area(area_id: int, _: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
...
def delete_area(area_id: int, _: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
