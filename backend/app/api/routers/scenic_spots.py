...
@router.get("")
def list_spots(_: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
    return {"items": ScenicSpotService(db).list_all()}


@router.get("/{spot_id}", response_model=ScenicSpotRead)
def get_spot(spot_id: int, _: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
...
def delete_spot(spot_id: int, _: object = Depends(require_authenticated_user), db: Session = Depends(get_db)):
