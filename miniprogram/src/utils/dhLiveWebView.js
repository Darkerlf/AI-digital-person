...
export function isSafeDhLiveWebViewUrl(url, platform = currentPlatform()) {
  const parsed = parseUrl(url)
  if (!parsed) return false
  if (parsed.protocol === 'https') return true
  return isDevtoolsPlatform(platform) && parsed.protocol === 'http' && LOCAL_HOSTS.has(parsed.hostname)
}

export function webViewBlockedReason(url, platform = currentPlatform()) {
